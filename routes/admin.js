...
router.post('/setftpserverdata', async (request, response) => {
    ...
    try {
        ...
        response.render('admin/index', { user: request.user, alert_info: `Successfully changed FTP server details`, show_modal: true});
    } catch (error) {
        console.error(`There was an error when attempting to update the ftp server data in the bot database document ${error}`);
        response.status(500).json({ error: `There was an error when attempting to update the ftp server data in the bot database document: ${error}` });
    }
});
