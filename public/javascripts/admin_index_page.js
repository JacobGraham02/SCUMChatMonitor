...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let packageNameToDelete = '';

    document.querySelectorAll('.bot_command_list_item_delete').forEach(function(deleteButton) {
        deleteButton.addEventListener('click', function(event) {
            event.preventDefault();
            packageNameToDelete = this.getAttribute('data-package-name');
    ...
    document.getElementById('confirmDeleteButton').addEventListener('click', function() {
        ...
        // Perform the delete operation (e.g., send a request to the server)
        fetch(`/admin/deletecommand/${encodeURIComponent(packageNameToDelete)}`, {
            method: 'DELETE',
            headers: {
            ...
            if (response.ok) {
                successModal.find('.modal-body p').text(`The command "${packageNameToDelete}" was successfully deleted`);
                successModal.modal('show');
                // Optionally, remove the deleted item from the list
                document.querySelector(`a[data-package-name="${packageNameToDelete}"]`).closest('li').remove();
            } else {
                successModal.find('.modal-body p').text(`The command "${packageNameToDelete} was not deleted`);
                successModal.modal('show');
            }
        }).catch(error => {
