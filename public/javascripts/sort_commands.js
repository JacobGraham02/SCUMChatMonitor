document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderCommands(commandList, currentPage = 1) {
        ...
        for (let i = start; i < end; i++) {
            ...
            commandContainer.id = 'current_command_container';
            const anchor = document.createElement('a');
            anchor.href = `command/${encodeURIComponent(command.package_name)}`;
            anchor.textContent = command.package_name;
            ...
            checkbox.value = command.package_name;
    ...
    function updatePagination(totalCommands, currentPage) {
        ...
        firstPageItem.classList.add('commands_pagination_list_item');
        ...
        for (let page = startPage; page <= endPage; page++) {
            ...
            paginationItem.classList.add('commands_pagination_list_item');
            if (page === currentPage) {
        ...
        lastPageItem.classList.add('commands_pagination_list_item');
    ...
    searchInput.addEventListener('input', function () {
        ...
        filteredCommands = serverCommands.filter(command =>
            command.package_name.toLowerCase().includes(query)
        );
