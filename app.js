...
async function establishFtpConnection(config) {
    for (let attempt = 1; ; attempt++) {
        try {
            ...
            console.error(`FTP attempt ${attempt} failed: ${err.message}`);
            message_logger.logError(`FTP attempt ${attempt} failed: ${err.message}`);

            if (attempt === maxRetries) {
                console.error('Max retries reached — giving up.');
                message_logger.logError('Max retries reached — giving up.');
                throw err;
            }

            const delay = retryDelay * attempt;
            console.log(`Retrying in ${delay/1000}s…`);
            message_logger.logError(`Retrying in ${delay/1000}s…`);
            await sleep(delay);
        }
    }
}
